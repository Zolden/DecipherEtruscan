# -*- coding: utf-8 -*-
"""Предрегистрация №1: перенос MDL-морфологии на Капуанскую табличку.

Правила заморожены в validation/prereg_tcap_morphology.md (коммит ДО
...
Запуск (единственный):
  PYTHONIOENCODING=utf-8 PYTHONHASHSEED=0 .venv/Scripts/python.exe tools/etr_prereg_tcap.py
"""
import math
import os
import pickle
...
OUT_LOG = os.path.join('logs', 'etr_prereg_tcap.log')
LOG = []
...
def main():
    ...
    log('=== Предрегистрация №1: MDL-морфология → Капуанская табличка ===')
    log('правила: validation/prereg_tcap_morphology.md (заморожены до прогона)')

    tr_freq, te_freq = Counter(), Counter()
    n_tr = n_te = 0
    for r in view:
        is_te = r['artifact_id'] == 'ART:TCap'
    ...
    log(f'train (без ART:TCap): {n_tr} записей, {N} токенов, '
        f'{len(types_sorted)} типов; TCap: {n_te} записей, {N_te} токенов, '
        f'{len(te_freq)} типов')
